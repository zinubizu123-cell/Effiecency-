...
export class OpenRouterAgent {
    ...
    try {
      ...
      for await (const message of this.sessionManager.getMessageIterator(session.sessionDbId)) {
        ...
        if (message.type === 'observation') {
          ...
          const obsResponse = await this.queryOpenRouterMultiTurn(session.conversationHistory, apiKey, baseUrl, model, siteUrl, appName);
          ...
          const summaryResponse = await this.queryOpenRouterMultiTurn(session.conversationHistory, apiKey, baseUrl, model, siteUrl, appName);
  ...
  private getOpenRouterConfig(): { apiKey: string; baseUrl: string; model: string; siteUrl?: string; appName?: string } {
    ...
    // Base URL: allows pointing at local OpenAI-compatible servers (e.g. llama.cpp)
