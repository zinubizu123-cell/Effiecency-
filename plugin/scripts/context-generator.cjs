...
`+JSON.stringify(o,null,2):m=" "+this.formatData(o));let l="";if(n){let{sessionId:T,memorySessionId:A,correlationId:S,...p}=n;Object.keys(p).length>0&&(l=` {${Object.entries(p).map(([g,h])=>`${g}=${h}`).join(", ")}}`)}let E=`[${i}] [${a}] [${d}] ${u}${s}${l}${m}`;if(this.logFilePath)try{(0,I.appendFileSync)(this.logFilePath,E+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),m=u?`${u[1].split("/").pop()}:${u[2]}`:"unknown",l={...s,location:m};return this.warn(e,`[HAPPY-PATH] ${t}`,l,n),o}},_=new J;var Ut={};function Dt(){return typeof __dirname<"u"?__dirname:(0,f.dirname)((0,ge.fileURLToPath)(Ut.url))}var Mt=Dt();function vt(){if(process.env.CLAUDE_MEM_DATA_DIR)return process.env.CLAUDE_MEM_DATA_DIR;let r=(0,f.join)((0,z.homedir)(),".claude-mem"),e=(0,f.join)(r,"settings.json");try{if((0,w.existsSync)(e)){let{readFileSync:t}=require("fs"),s=JSON.parse(t(e,"utf-8")),n=s.env??s;if(n.CLAUDE_MEM_DATA_DIR)return n.CLAUDE_MEM_DATA_DIR}}catch{}return r}var N=vt(),L=process.env.CLAUDE_CONFIG_DIR||(0,f.join)((0,z.homedir)(),".claude"),es=(0,f.join)(L,"plugins","marketplaces","thedotmack"),ts=(0,f.join)(N,"archives"),ss=(0,f.join)(N,"logs"),rs=(0,f.join)(N,"trash"),ns=(0,f.join)(N,"backups"),os=(0,f.join)(N,"modes"),is=(0,f.join)(N,"settings.json"),Te=(0,f.join)(N,"claude-mem.db"),as=(0,f.join)(N,"vector-db"),ds=(0,f.join)(N,"observer-sessions"),cs=(0,f.join)(L,"settings.json"),us=(0,f.join)(L,"commands"),ms=(0,f.join)(L,"CLAUDE.md");function fe(r){(0,w.mkdirSync)(r,{recursive:!0})}function Se(){return(0,f.join)(Mt,"..")}var be=require("crypto");var xt=3e4;function P(r,e,t){return(0,be.createHash)("sha256").update([r||"",e||"",t||""].join("\0")).digest("hex").slice(0,16)}function H(r,e,t){let s=t-xt;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,s)}function Q(r){if(!r)return[];try{let e=JSON.parse(r);return Array.isArray(e)?e:[String(e)]}catch{return[r]}}var O="claude";function kt(r){return r.trim().toLowerCase().replace(/\s+/g,"-")}function y(r){if(!r)return O;let e=kt(r);return e?e==="transcript"||e.includes("codex")?"codex":e.includes("cursor")?"cursor":e.includes("claude")?"claude":e:O}function he(r){let e=["claude","codex","cursor"];return[...r].sort((t,s)=>{let n=e.indexOf(t),o=e.indexOf(s);return n!==-1||o!==-1?n===-1?1:o===-1?-1:n-o:t.localeCompare(s)})}function $t(r,e){return{customTitle:r,platformSource:e?y(e):void 0}}var X=class{db;constructor(e=Te){e!==":memory:"&&fe(N),this.db=new Oe.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.addSessionPlatformSourceColumn(),this.addObservationModelColumns()}initializeSchema(){this.db.run(`
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,type:i,concepts:a,files:d}=t,u=s==="date_asc"?"ASC":"DESC",m=n?`LIMIT ${n}`:"",l=e.map(()=>"?").join(","),E=[...e],T=[];if(o&&(T.push("project = ?"),E.push(o)),i)if(Array.isArray(i)){let p=i.map(()=>"?").join(",");T.push(`type IN (${p})`),E.push(...i)}else T.push("type = ?"),E.push(i);if(a){let p=Array.isArray(a)?a:[a],b=p.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");E.push(...p),T.push(`(${b.join(" OR ")})`)}if(d){let p=Array.isArray(d)?d:[d],b=p.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");p.forEach(g=>{E.push(`%${g}%`,`%${g}%`)}),T.push(`(${b.join(" OR ")})`)}let A=T.length>0?`WHERE id IN (${l}) AND ${T.join(" AND ")}`:`WHERE id IN (${l})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${A}
    ...
    `).get(e).count}createSDKSession(e,t,s,n,o){let i=new Date,a=i.getTime(),d=$t(n,o),u=d.platformSource??O,m=this.db.prepare(`
    ...
    `).run(e,t,s,n.toISOString(),o).lastInsertRowid}getUserPrompt(e,t){return this.db.prepare(`
      ...
      LIMIT 1
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i,a){let d=i??Date.now(),u=new Date(d).toISOString(),m=P(e,s.title,s.narrative),l=H(this.db,m,d);if(l)return{id:l.id,createdAtEpoch:l.created_at_epoch};let T=this.db.prepare(`
    ...
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(m.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a,d){let u=a??Date.now(),m=new Date(u).toISOString();return this.db.transaction(()=>{let E=[],T=this.db.prepare(`
      ...
      `);for(let S of s){let p=P(e,S.title,S.narrative),b=H(this.db,p,u);if(b){E.push(b.id);continue}let g=T.run(e,t,S.type,S.title,S.subtitle,JSON.stringify(S.facts),S.narrative,JSON.stringify(S.concepts),JSON.stringify(S.files_read),JSON.stringify(S.files_modified),o||null,i,p,m,u,d||null);E.push(Number(g.lastInsertRowid))}let A=null;if(n){let p=this.db.prepare(`
        ...
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         ...
         generated_by_model)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let g of s){let h=P(e,g.title,g.narrative),_e=H(this.db,h,l);if(_e){A.push(_e.id);continue}let Ot=S.run(e,t,g.type,g.title,g.subtitle,JSON.stringify(g.facts),g.narrative,JSON.stringify(g.concepts),JSON.stringify(g.files_read),JSON.stringify(g.files_modified),a||null,d,h,E,l,m||null);A.push(Number(Ot.lastInsertRowid))}let p;if(n){let h=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,d,E,l);p=Number(h.lastInsertRowid)}return this.db.prepare(`
    ...
    `).all(...u)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],d,u;if(e!==null){let p=`
      ...
      `,b=`
        ...
        ORDER BY id ASC
        LIMIT ?
      `;try{let g=this.db.prepare(p).all(e,...a,s+1),h=this.db.prepare(b).all(e,...a,n+1);if(g.length===0&&h.length===0)return{observations:[],sessions:[],prompts:[]};d=g.length>0?g[g.length-1].created_at_epoch:t,u=h.length>0?h[h.length-1].created_at_epoch:t}catch(g){return _.error("DB","Error getting boundary observations",void 0,{error:g,project:o}),{observations:[],sessions:[],prompts:[]}}}else{let p=`
      ...
      `,b=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let g=this.db.prepare(p).all(t,...a,s),h=this.db.prepare(b).all(t,...a,n+1);if(g.length===0&&h.length===0)return{observations:[],sessions:[],prompts:[]};d=g.length>0?g[g.length-1].created_at_epoch:t,u=h.length>0?h[h.length-1].created_at_epoch:t}catch(g){return _.error("DB","Error getting boundary timestamps",void 0,{error:g,project:o}),{observations:[],sessions:[],prompts:[]}}}let m=`
      SELECT *
      FROM observations
...
`):null}function jt(r,e,t,s){let n=[];n.push(...Xe(r));let o="";for(let i of e)if(i.type==="summary"){let a=i.data,d=ce(a.displayTime);n.push(...We(a,d))}else{let a=i.data,d=ue(a.created_at),m=d!==o?d:"";if(o=d,t.has(a.id)){let E=pt(a,s);n.push(...Be(a,m,E,s))}else n.push(Ge(a,m,s))}return n}function Gt(r,e,t,s,n){let o=[];o.push(...tt(r));let i=null,a="";for(let d of e)if(d.type==="summary"){i=null,a="";let u=d.data,m=ce(u.displayTime);o.push(...ot(u,m))}else{let u=d.data,m=_t(u.files_modified,n,u.files_read),l=ue(u.created_at),E=l!==a;a=l;let T=t.has(u.id);if(m!==i&&(o.push(...st(m)),i=m),T){let A=pt(u,s);o.push(...nt(u,l,E,A,s))}else o.push(rt(u,l,E,s))}return o.push(""),o}function Bt(r,e,t,s,n,o){return o?Gt(r,e,t,s,n):jt(r,e,t,s)}function lt(r,e,t,s,n){let o=[],i=Xt(r);for(let[a,d]of i)o.push(...Bt(a,d,e,t,s,n));return o}function Et(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function gt(r,e){let t=[];return e?(t.push(...$("Investigated",r.investigated,c.blue)),t.push(...$("Learned",r.learned,c.yellow)),t.push(...$("Completed",r.completed,c.green)),t.push(...$("Next Steps",r.next_steps,c.magenta))):(t.push(...k("Investigated",r.investigated)),t.push(...k("Learned",r.learned)),t.push(...k("Completed",r.completed)),t.push(...k("Next Steps",r.next_steps))),t}function Tt(r,e){return e?it(r):Ye(r)}function ft(r,e,t){return!B(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?at(r.totalDiscoveryTokens,r.totalReadTokens):qe(r.totalDiscoveryTokens,r.totalReadTokens)}var Wt=St.default.join((0,bt.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function Yt(){try{return new X}catch(r){if(r.code==="ERR_DLOPEN_FAILED"){try{(0,ht.unlinkSync)(Wt)}catch(e){_.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return _.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function qt(r,e){return e?dt(r):Ve(r)}function Vt(r,e,t,s,n,o,i){let a=[],d=re(e);a.push(...ct(r,d,s,i));let u=t.slice(0,s.sessionCount),m=Ue(u,t),l=ae(e,m),E=xe(e,s.fullObservationCount);a.push(...lt(l,E,s,n,i));let T=t[0],A=e[0];Et(s,T,A)&&a.push(...gt(T,i));let S=ie(e,s,o,n);a.push(...Tt(S,i)),a.push(...ft(d,s,i));let p=5e5,b=a.join(`
`).trimEnd();return b.length>p?b.slice(0,p)+`

... [context truncated \u2014 exceeded `+p.toLocaleString()+" char limit]":b}async function me(r,e=!1){let t=ee(),s=r?.cwd??process.cwd(),n=Ne(s),o=r?.platform_source,i=r?.projects||[n],a=500,d=50;r?.full&&(t.totalObservationCount=a,t.sessionCount=d);let u=Yt();if(!u)return"";try{let m=i.length>1?Me(u,i,t,o):ne(u,n,t,o),l=i.length>1?ve(u,i,t,o):oe(u,n,t,o);return m.length===0&&l.length===0?qt(n,e):Vt(n,m,l,t,s,r?.session_id,e)}finally{u.close()}}0&&(module.exports={generateContext});
